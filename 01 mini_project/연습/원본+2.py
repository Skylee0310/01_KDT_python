...
2) 메뉴 선택
    2-1) 샌드위치 (메뉴 출력)
        ...
        + 세트 메뉴 추가
'''
#함수1 : 입력이 잘못되었을 경우 메시지를 출력
def wrong_num() :
    print("▶ 잘못된 번호를 입력하셨습니다.")

#함수2 : 인수가 decimal(십진수)일 때 정수(int)로 반환.
def num_int(n):
    #n은 str
    if n.isdecimal() :
        n=int(n)
...
# 문제점
#1) 코드가 너무 길어진 탓에 가독성이 떨어짐.
#2) [샌드위치] 선택(choice2, 3) 중에 잘못된 번호를 입력하면 [매장이용] 항목으로 돌아가 버림.
#3) 샌드위치 개수 선택을 어떻게 해야할지 모르겠음.
#4) 도중에 종료할 수 없음.