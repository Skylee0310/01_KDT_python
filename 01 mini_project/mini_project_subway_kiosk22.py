...
def print_result() :
    ...
    time.sleep(3)
    writing_ =f"[주문 내역]"
    writing_menu = f"◆ 메뉴 : {sandwich}"
    writing_set = f"◆ 세트 추가 : {set_}"
    writing_price = f"◆ 가격 : {price}"
    writing_here_togo = f"◆ 매장 이용 여부 : {here_togo}"
    file = "subway_menu.txt"
    with open(file, mode="a", encoding="utf-8") as f :
        writingList = [writing_,writing_menu, writing_set, writing_price, writing_here_togo]
    ...
    print()
    print("주문을 종료하시겠습니까?")
    order_end = input("주문을 종료하시려면 'Y' 혹은 'y'를 입력하세요 : ")
    if order_end in ('Y', 'y') :
        print("=" * 80)
        print("[로딩 중]")
        print("=" * 80)
